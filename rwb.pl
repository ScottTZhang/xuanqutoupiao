...
if ($action eq "base") { 
  ...
  #
  print "<div id=\"color\" style=\"width:100\%; height:6\%\"></div>";
  print "<div id=\"comm_color\" style=\"width:100\%; height:20px;border:solid black 1px\">commitees</div>";
  print "<div id=\"indi_color\" style=\"width:100\%; height:20px;border:solid black 1px\">individuals</div>";
  #
  #
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  if ($user eq "anon") {
    ...
    print "<div>
		...
		<input type=\"checkbox\" id=\"comm\" checked value=\"committees\">committees&nbsp;
		<input type=\"checkbox\" id=\"cand\" value=\"candidates\" >candidates&nbsp;
		<input type=\"checkbox\" id=\"indi\" value=\"individuals\">individuals&nbsp;
		<input type=\"checkbox\" id=\"opin\" value=\"opinions\">opinions&nbsp;<br>
		<label>Cycle:</lable>
		...
		<option selected=\"selected\" value=\"1112\">1112</option>
		...
		<input type=\"submit\" onclick=\"ViewShift();\" value=\"Query\"> 
	</div>";
...
if ($action eq "near") {
 ...
 my (@str2,$error2) = Committees_Agg($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
	...
	print $str;
     ...
     if (!$error2) {	
	while(	$str2[0][0] + $str2[1][0] ==0)
  	{ $latne+=1;
	$longne+=1;
	$latsw-=1;
	$longsw-1;
	(@str2,$error2) = Committees_Agg($latne,$longne,$latsw,$longsw,$cycle,$format);
	}
	if($str2[0][0] > $str2[1][0]){print "larger amount is comm_REP: ",$str2[0][0];}else {print "larger amount is comm_DEM: ",$str2[1][0];}
	#else {$amount = "DEM",$str2[1][0];}
	print "<input id='comm_sumVal1' type='hidden' value='".$str2[0][0]."' />";
	print "<input id='comm_sumVal2' type='hidden' value='".$str2[1][0]."' />";
        	}
  }
...
my (@str2,$error2) = Individuals_Agg($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
   ...
   if (!$error2) {
	...
	$latsw-=1;
	$longsw-1;
	(@str2,$error2) = Individuals_Agg($latne,$longne,$latsw,$longsw,$cycle,$format);
	}
	if($str2[0][0] > $str2[1][0]){print "larger amount is indi_REP: ",$str2[0][0];}else {print "larger amount is indi_DEM: ",$str2[1][0];}
	print "<input id='indi_sumVal1' type='hidden' value ='".$str2[0][0]."' />";
	print "<input id='indi_sumVal2' type='hidden' value ='".$str2[1][0]."' />";
  }
}
  if ($what{opinions}) {
    my ($str,$error) = Opinions($latne,$longne,$latsw,$longsw,$cycle,$format);
    print "    ",$latne-5," ",$longne-5," ",$latsw+5," ",$longsw+5;
    my (@str3,$error3) = Opinions_Avg($latne+1,$longne+1,$latsw-1,$longsw-1,$cycle,$format);
    my (@str4,$error4) = Opinions_Stddev($latne+1,$longne+1,$latsw-1,$longsw-1,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
	print "<h2>Nearby committees</h2>$str";
      } else {
	print $str;
     ...
     if (!$error3) {
        my $avgnum = sprintf("%.4f", $str3[0][0]);
	print "<input id='opiavg' type='hidden' value='$avgnum' />";
      	}
     if (!$error3) {
	my $stdnum = sprintf("%.4f", $str4[0][0]);
	#print "<input id='opiavg' type='hidden' value='$str3[0][0]' />";
	print "<input id='opistd' type='hidden' value='$stdnum' />";
      	}
...
{ 
  ...
  #FIXME: Check if you need to also check for manage-user permissions here.
  if (!UserCan($user,"invite-users") && !UserCan($user,"manage-users")) { 
    ...
    if (!$run) {
      my @perms; 
      eval {
	@perms = ExecSQL($dbuser, $dbpasswd, "select action from rwb_permissions where name=?","COL",$user);
	};
     #foreach(@perms){print $_;}
	print start_form(-name=>'InviteUser'),
      ...
      my @validpermissions=param('permissions');
      my $error;
      $error=UserInvite($email,@validpermissions);
      if ($error) {
        print "Can't invite user because: $error";
      } else {
        print "User $user invited user $email\n";
      }
    }
  }
...
sub UserInvite {
	...
	#
	my $code = time;
	eval { ExecSQL($dbuser,$dbpasswd,"insert into rwb_invitations (email,code,permissions,referer,expire) values (?,?,?,?,?)",$email,$email,$code,join(",",@validpermissions),$user,"no");};
	
	my $subject=" RWB Invitation";
	my $content="Please click on the link below to visit a page to create your account for RWB: \nhttp://murphy.wot.eecs.northwestern.edu/~tzh240/rwb/rwb.pl?act=validate&code=".$code;
...
if($action eq "validate")
{
    ...
    my $error;
    my $exist;
    ($exist,$error) = CheckExpire($code);
    #print $exist; #1
    #print "this is error: ",$error,"!\n";
    if($error){print h2("Cannot validate because: $error.");}
    if(!$exist){print "expired.";}
    else 
	{
	if(!$run)
		{
		my $email;
		    my $referer;
		    my $error;
	  	    ($email,$referer,$error) = GetInvitationInfo($code);
		    #print $email,$referer;
      		    print start_form(-name=>'Account Creation'),
      h2('Create an account for RWB'),
        "Name:",textfield(-name=>'user'),p,
		...
		my $email = param("email");
		#print $user,$password,$referer,$email;
		$error = UserAdd($user,$password,$email,$referer);
		if ($error)
		{
		...
		{
			print "<h3>add user success.</h3>";
			my @rows1;
			eval { @rows1 = ExecSQL($dbuser, $dbpasswd, "select email,referer,permissions from rwb_invitations where code=?","table",url_param('code'));};
			my @perm_array = split(",",$rows1[0][2]);
			foreach my $thisPerm (@perm_array) {
		
				$error=GiveUserPerm(param("user"),$thisPerm);
      				if ($error) {
        				print "Can't add permission to user because: $error";
      				} else {
        				print "<p>Gave user $user permission $thisPerm.</p>";
      				}
				print "<br />";
			}
		}
...
#my $code = @_;
	...
	eval
	{
		@exist = ExecSQL($dbuser,$dbpasswd,"select 1 from rwb_invitations where expire='no' and code=?","COL",@_);
		#print h2("@exist");
	};
	if ($@)
	{
...
sub ExpireInvitation
{
	eval
	{
		ExecSQL($dbuser,$dbpasswd,"update rwb_invitations set expire='yes' where code=?",undef,@_);
	};
	return $@;
}

sub GetInvitationInfo
{
	my @info;
	eval
	{
		...
		ExecSQL($dbuser,$dbpasswd,
			"select email,referer from rwb_invitations where code=?",
			"ROW",@_);
	};
	if ($@)
	{
...
if (($action eq "give-opinion-data") && (UserCan($user,"give-opinion-data")))
{
  	print "<script src=\"http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js\" type=\"text/javascript\"></script>";
  	print "<script src=\"http://maps.google.com/maps/api/js?sensor=true\" type=\"text/javascript\"></script>";
  	print "<script type=\"text/javascript\" src=\"rwb.js\"> </script>";
	if (!$run)
	{
		my %lables = ('1'=>'Blue','-1'=>'Red','0'=>'White');
		print h2("Give your opinion"),
		start_form(-name=>'opinion_form'),
		#popup_menu(-name=>'opinion',-values=>['1','-1','0'],-lables=>\%lables),
		"<select name=\"opinion\">
		<option value=\"1\">Blue</option>
		<option value=\"0\" selected=\"selected\">White</option>
		<option value=\"-1\">Red</option>
		</select>",
		"<p id=\"demo\">this is a p.</p>",
		hidden(-name=>'run',-default=>['1']),
		hidden(-name=>'act',-default=>'give-opinion-data'),
		hidden(-name=>'latitude'),
		hidden(-name=>'longitude'),
		button(-value=>'First: Retrieve Geolocation Information',-onclick=>"Submit()"),
		submit(-name=>'submit_opinion',-value=>'Next: Submit Opinion'),
		end_form,
		hr;
	}
	else
	{
		...
		#$latitude = sprintf("%.5s",$latitude);
		#$longitude = sprintf("%.5s",$longitude);
		print h2("$user,$opinion,$latitude,$longitude");
		my $error;
		$error = GiveOpinion($user,$opinion,$latitude,$longitude);
		if ($error)
		{
			print h2("Cannot give opinions because: $error");
		}
		else
		{
			print h2("Your opinion is submitted.");
		}
	}
	print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
...
sub GiveOpinion
{
	eval
	{
		ExecSQL($dbuser,$dbpasswd,"insert into rwb_opinions (submitter,color,latitude,longitude) values (?,?,?,?)",undef,@_);
	};
	return $@;
}
...
#
#
# check availablity with invitation code
...
sub Committees {
  ...
  my @rows;
  my @cycle= split(",",$cycle);
  my $qmarks = join( ",",map{"?"}@cycle);
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle in (".$qmarks.") and latitude>? and latitude<? and longitude>? and longitude<?",undef,@cycle,$latsw,$latne,$longsw,$longne);
  };
...
sub Committees_Agg {
  ...
  my @rows;
  my @cycle=split(",",$cycle);
  my $newcycle=join(",",map{"'".$_."'"}@cycle);
  eval { 
@rows = ExecSQL($dbuser, $dbpasswd,"select sum(sumt) from (select cmte_pty_affiliation,sum(transaction_amnt) as sumt from cs339.comm_to_comm  natural join cs339.cmte_id_to_geo natural join cs339.committee_master where cycle in ($newcycle) and latitude>? and  latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('REP', 'DEM') group by cmte_pty_affiliation UNION ALL select cmte_pty_affiliation, sum(transaction_amnt) as sumt from cs339.comm_to_cand natural join cs339.cmte_id_to_geo natural join cs339.committee_master where cycle in ($newcycle) and  latitude>? and latitude<?  and longitude>? and longitude<? and cmte_pty_affiliation in ('REP', 'DEM') group by cmte_pty_affiliation) group by cmte_pty_affiliation",undef,$latsw,$latne,$longsw,$longne,$latsw,$latne,$longsw,$longne);
};
return @rows;
}
...
sub Individuals {
  ...
  my $qmarks = join( ",",map{"?"}@cycle);
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_to_geo where cycle in(".$qmarks.") and latitude>? and latitude<? and longitude>? and longitude<?",undef,@cycle,$latsw,$latne,$longsw,$longne);
  };
...
sub Individuals_Agg {
  ...
  my $newcycle=join(",",map{"'".$_."'"}@cycle);
  eval {
@rows = ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt) from cs339.individual natural join cs339.ind_to_geo natural join cs339.committee_master where cycle in ($newcycle) and cmte_pty_affiliation in ('REP', 'DEM') and latitude>? and latitude<? and longitude>? and longitude<? group by cmte_pty_affiliation",undef,$latsw,$latne,$longsw,$longne);
};
return @rows;
=start
  if ($@) { 
